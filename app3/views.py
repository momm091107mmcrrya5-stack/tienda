...
class StoresListView(ListView):
    ...
    template_name = "filtro_store.html"
   
    def get_queryset(self):
        print('*************************')
        palabra_clave=self.request.GET.get("store",'')
        print('=================',palabra_clave)
...
def delete_store(request):
    ...
    if request.method == 'POST':
        ...
        if store != "":
            try:
                obj = stores.objects.get(name__iexact=stores)
                obj.delete()
                context['mensaje'] = f"Tienda '{store}' eliminada correctamente."
            except stores.DoesNotExist:
                context['error'] = f"No se encontró la tienda '{store}'."
